...
def test_report_rows_project_the_full_log_newest_first() -> None:
    projection = build_site.report_projection()

    assert len(projection) == 20
    observed = [row["observed_at"] for row in projection]
    assert observed == sorted(observed, reverse=True)
    assert all(row["published"] == "not available" for row in projection)
    retained = [row for row in projection if row["bundle_path"]]
    unavailable = [row for row in projection if not row["bundle_path"]]
    assert len(retained) == 18
    assert all(row["report_digest"].startswith("0x") for row in retained)
    assert all(row["bundle_path"].startswith("/data/") for row in retained)
    assert len(unavailable) == 2
    assert all(row["report_digest"] == "not available" for row in unavailable)
    assert {row["network"] for row in projection} == {
        "X Layer mainnet",
        "X Layer testnet",
    }
