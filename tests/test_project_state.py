...
def test_project_state_is_assembled_from_verified_repository_facts(tmp_path: Path) -> None:
    ...
    assert state["reports"]["retained_verified_policy_bundle_count"] == 0
    assert state["reports"]["confirmed_policy_bundle_count"] == 0
...
def test_judge_page_obeys_the_static_site_csp() -> None:
    ...
    assert page.count('data-policy-state="UNVERIFIABLE"') == 2
