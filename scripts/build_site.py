...
def _network_for_transaction(
    transaction_hash: str, committed: dict[str, object]
) -> tuple[str, str, str]:
    wanted = transaction_hash.removeprefix("0x").lower()
    for key, label, chain_id in (
        ("mainnet", "X Layer mainnet", "196"),
        ("testnet", "X Layer testnet", "1952"),
    ):
        network = committed.get(key)
        if not isinstance(network, dict):
            continue
        pubs = network.get("pubs")
        values = pubs.values() if isinstance(pubs, dict) else ()
        if any(str(value).lower() == wanted for value in values):
            explorer = network.get("explorer")
            if not isinstance(explorer, str):
                raise SystemExit(f"facts.json has no explorer for {key}")
            return label, chain_id, explorer
        first = network.get("first_publication_tx")
        if isinstance(first, str) and first.lower() == wanted:
            explorer = network.get("explorer")
            if not isinstance(explorer, str):
                raise SystemExit(f"facts.json has no explorer for {key}")
            return label, chain_id, explorer
    raise SystemExit(f"report transaction {transaction_hash} has no network fact")


def _matching_bundle(
    row: dict[str, object], chain_id: str
    ...
    for path, bundle in _bundles():
        ...
        if (
            report.get("epoch_id") == row.get("epoch_id")
            and report.get("sequence") == row.get("sequence")
            and report.get("state") == row.get("state")
            and report.get("observed_at") == row.get("observed_at")
            and policy_id == row.get("policy")
        ):
    ...
    if len(candidates) == 1:
        return candidates[0]
    marker = f"eip155-{chain_id}-"
    marked = [
        candidate for candidate in candidates if candidate[0].name.startswith(marker)
    ]
    if len(marked) == 1:
        return marked[0]
    if chain_id == "196":
        mainnet = [
            candidate for candidate in candidates if "-mainnet" in candidate[0].stem
        ]
        if len(mainnet) == 1:
            return mainnet[0]
    else:
        testnet = [
            candidate for candidate in candidates if "-mainnet" not in candidate[0].stem
        ]
        if len(testnet) == 1:
            return testnet[0]
    raise SystemExit(
        f"report {row.get('transaction_hash')} matches multiple retained bundles"
    )


def report_projection() -> list[dict[str, str]]:
    committed = _committed_facts()
    reports = _stats().get("reports")
    if not isinstance(reports, list):
        raise SystemExit("stats.json has no reports list")
    projection = []
    for row in reports:
        if not isinstance(row, dict):
            raise SystemExit("stats.json report rows must be objects")
        transaction_hash = row.get("transaction_hash")
        if not isinstance(transaction_hash, str):
            raise SystemExit("stats.json report has no transaction hash")
        network, chain_id, explorer = _network_for_transaction(
            transaction_hash, committed
        )
        matched = _matching_bundle(row, chain_id)
        if matched is None:
            digest = "not available"
            bundle_path = ""
        else:
            path, bundle = matched
            canonical = bundle.get("report_canonical")
            if not isinstance(canonical, str):
                raise SystemExit(f"{path}: report_canonical is unavailable")
            digest = "0x" + hashlib.sha256(canonical.encode("utf-8")).hexdigest()
            bundle_path = "/data/" + path.name
        projection.append(
            {
                "asset": "USTB",
                "published": "not available",
                "observed_at": str(row.get("observed_at", "")),
                "state": str(row.get("state", "not available")),
                "report_digest": digest,
                "transaction_hash": transaction_hash,
                "transaction_url": f"{explorer}/tx/{transaction_hash}",
                "bundle_path": bundle_path,
                "network": network,
                "sequence": str(row.get("sequence", "not available")),
            }
        )
    projection.sort(
        key=lambda item: (item["observed_at"], item["transaction_hash"]),
        reverse=True,
    )
    return projection


def _report_rows_html() -> str:
    rows = []
    chip_classes = {
        "CONFIRMED": "chip-live",
        "STALE": "chip-blocked",
    ...
    for row in report_projection():
        state = escape(row["state"])
        chip = chip_classes.get(row["state"], "chip-unverifiable")
        digest = escape(row["report_digest"])
        if row["bundle_path"]:
            report_hash = f'<a href="{escape(row["bundle_path"])}">{digest}</a>'
        else:
            report_hash = digest
        rows.append(
            "<tr>"
            f'<td data-label="Asset"><strong>{escape(row["asset"])}</strong>'
            f'<span class="td-sub mono">{escape(row["network"])} · seq '
            f"{escape(row['sequence'])}</span></td>"
            f'<td data-label="Published" class="mono">{escape(row["published"])}</td>'
            f'<td data-label="State"><span class="chip {chip}"><span class="dot"></span>'
            f"{state}</span></td>"
            f'<td data-label="Report hash" class="mono report-hash">{report_hash}</td>'
            f'<td data-label="Transaction" class="mono report-tx"><a href="{escape(row["transaction_url"])}">'
            f"{escape(row['transaction_hash'])}</a></td>"
            "</tr>"
        )
    return "\n".join(rows)
...
def site_facts() -> dict[str, str]:
    ...
    return {
        ...
        "reports.rows": _report_rows_html(),
    }
