...
| T7 | **Hostile PDF** | **Not applicable in Phase 1.** No PDF path exists. USDY's unbounded archive remains cut; FOBXX's retained SEC N-MFP3 path is XML with per-source byte and depth limits plus DTD/entity/CDATA refusal (`touchstone/normalize/fobxx.py`). Page and archive expansion limits become required again if either source class is added |
